...
В [`paywall-dialog.tsx`](src/components/paywall/paywall-dialog.tsx) состав каждого
пакета (`responses`/`analyses`/`resumes`) выводится прямо внутри карточки тарифа,
а не отдельной строкой под селектором. Это сделано ради мобильной версии: иначе
сколько лимитов даёт пакет было видно только после прокрутки. По той же причине
список `PRO_BENEFITS` сокращён до коротких однострочных пунктов — весь paywall
должен помещаться на один экран телефона без скролла.
...
- **Батч** `BATCH_SIZE = 5`: за один запрос отправляем до 5 вакансий → 1 ответ-массив.
  Батч сознательно маленький: с полными описаниями (см. ниже) большой батч снова
  разбавляет внимание маленькой модели и провоцирует перепутывание вакансий между
  собой. Сам `/api/match` принимает до 10 (защитный потолок).
...
- **В скоринг идёт полное описание вакансии + `key_skills`.** Клиент по-прежнему
  шлёт `snippet` (короткий тизер из поиска), но `/api/match` на сервере обогащает
  каждую вакансию батча: тянет деталь через `getVacancy(id)` (бесплатный запрос к
  hh.ru, кэш `revalidate: 3600`), кладёт в `info` очищенное от HTML описание
  (в промпте обрезается до 2000 симв.) и добавляет `key_skills`. Если деталь
  недоступна (вакансия снята/архив) — остаётся snippet как fallback. Раньше
  скоринг работал только по snippet ради экономии токенов, но из-за этого
  strengths/gaps строились по обрывку текста; вход подорожал в ~2 раза
  (см. PRICING.md), зато текстовая оценка опирается на реальные требования.
- **Промпт матчинга «заземлён»** ([prompts.ts](src/lib/ai/prompts.ts),
  `buildMatchMessages`): жёсткие правила требуют, чтобы `strengths` опирались
  ТОЛЬКО на факты из резюме, а `gaps` — ТОЛЬКО на требования из полей самой
  вакансии, отсутствующие в резюме; при нехватке данных модель должна вернуть
  пустые массивы, а не догадки. Это ответ на реальный кейс: модель приписывала
  кандидату опыт из текста вакансии (B2B), а навык, который в резюме был
  (аналитика), записывала в gaps. Без этих правил маленькая модель (mistral-nemo)
  в батч-режиме генерирует «рекрутёрский текст» по вакансии, а не сравнение с резюме.
- **Лимиты промпта матчинга:** резюме обрезается до **10 000 симв.**
  (раньше 4000 — у длинных резюме «хвост» `rawText` не доходил до модели, и она
  не видела навыки, упомянутые дальше по тексту); `info` вакансии — до 2000 симв.;
  ответ — `maxTokens: 2500` ([route.ts](src/app/api/match/route.ts)) (раньше
  1200 ≈ 120 токенов на вакансию при батче 10 — модель «экономила слова», и был
  риск обрезанного JSON, при котором часть вакансий получала `ok=false`).
...
### 9.0.1. Отладка AI-промптов

Для диагностики спорных оценок есть закрытая от индексации страница:
`/debug/ai-prompts?vacancyId=<hh-id>&kind=match|cover`.

Она доступна только пользователю с активной auth/app-session cookie и собирает
prompt payload без вызова ИИ:

- **`kind=match`** — строит `messages` через тот же `buildMatchMessages()` и
  показывает `MatchVacancyInput`: `id`, `name`, `company`, `experience`,
  `keySkills`, `info`. Страница зеркалит серверное обогащение `/api/match`:
  базовые поля берутся из сохранённого снапшота таблицы `vacancies` (если
  карточка оценивалась/лайкалась), а описание и `key_skills` дотягиваются из
  детальной вакансии hh.ru — ровно как в production. Если ни снапшота, ни
  детали нет, показывается понятная ошибка.
- **`kind=cover`** — дотягивает детальную вакансию hh.ru и строит `messages` через
  `buildCoverLetterMessages()`, как делает `/api/cover-letter` для обычной hh.ru
  вакансии.
- В обоих режимах страница показывает `resumeContext` из текущего профиля
...
- **Контейнер и адаптив** — приложение mobile-first. На мобильном/планшете (`< lg`)
  контент — центрированная колонка `max-w-2xl`, навигация — плавающий «pill» внизу
  (`position: fixed`). На десктопе (`≥ lg`) включается **app-shell** с левым
  сайдбаром (см. §13.1). Ширину контента задаёт не корневой layout, а сам shell.

---

## 13.1. Десктопная раскладка (app-shell)

До `lg` интерфейс мобильный и **не меняется**. На `≥ lg` приложение разворачивается в
desktop-shell, чтобы контент не висел узкой полосой по центру с пустыми краями.

### Компоненты
- [`app-shell.tsx`](src/components/layout/app-shell.tsx) — обёртка всего приложения,
  рендерится в [`layout.tsx`](src/app/layout.tsx) **вместо** старого `max-w-2xl`
  контейнера. Клиентский компонент: знает `pathname` и сессию из стора.
  - **Фон `app-aurora`** перенесён на полноэкранную обёртку (`min-h-dvh w-full`),
    поэтому свечение заполняет весь вьюпорт, а не центральную полосу.
  - **Ширину контента задаёт shell, а НЕ корневой layout.** Это сознательно:
    app-экраны (лента/отклики/кабинет) держатся в читаемой колонке `max-w-2xl mx-auto`,
    а публичный лендинг (`/` без сессии) от этого кэпа отказывается и раскладывается
    во всю ширину как широкая посадочная.
  - **Где сайдбар:** только на app-маршрутах (`/` с сессией, `/liked`, `/profile`).
    На лендинге, `/legal/*`, `/vakansii/*` сайдбара нет (свой marketing-хедер или
    прежняя центрированная колонка).
- [`sidebar.tsx`](src/components/layout/sidebar.tsx) — десктопная навигация
  (`hidden lg:flex`, `sticky top-0 h-dvh`). Зеркалит вкладки и бейдж откликов из
  [`bottom-nav.tsx`](src/components/layout/bottom-nav.tsx); внизу — `ProButton`.
- [`bottom-nav.tsx`](src/components/layout/bottom-nav.tsx) получил `lg:hidden` — на
  десктопе его заменяет сайдбар. Шапки `/profile` и `/liked` тоже `lg:hidden`, чтобы
  бренд не дублировался с сайдбаром.

### Лендинг ([`guest-home.tsx`](src/components/landing/guest-home.tsx))
- Хедер: brand + nav-якоря (`#how-it-works`, `#audience`, `#faq`) + CTA, центрирован
  в `max-w-6xl`. Nav-ссылки и второй CTA — под `lg:` (на мобиле скрыты).
- **Hero в две колонки на `lg`** (текст слева, `ProductPreview` справа). Приём: hero
  и `ProductPreview` обёрнуты в `<div className="contents lg:grid lg:grid-cols-2 …">`.
  `display:contents` на мобиле делает обёртку «прозрачной» — секции рендерятся ровно
  как раньше (стопкой, превью полосой); на `lg` обёртка становится grid'ом. Так
  мобильная вёрстка не трогается, а десктоп получает другую раскладку без дублей.
- Остальные секции обёрнуты в `max-w-6xl mx-auto` (кэп только на десктопе; ниже `lg`
  это no-op), чтобы на широком экране они не растягивались во всю ширину.

### Принцип (почему мобильную верстку это не ломает)
Все desktop-правки — это либо `lg:`-варианты классов, либо кэпы шириной, которые ниже
`lg` являются no-op. Поэтому телефон/планшет видят ровно прежний интерфейс, а новая
композиция включается только на широком экране.

### App-экраны на десктопе
На `lg` AppShell отдаёт app-маршрутам полную ширину (`lg:max-w-none`), и каждый экран
сам задаёт свою desktop-композицию:
- **Лента** ([`home-client.tsx`](src/components/home/home-client.tsx)) — master–detail.
  Шапка-тулбар во всю ширину контейнера (слева — счётчик «Найдено N…», справа Pro +
  «Фильтры»); бренд скрыт (`lg:hidden`, он в сайдбаре). Контейнер растёт по ширине
  экрана: `lg:max-w-3xl` → `xl:max-w-6xl` → `2xl:max-w-7xl`.
  - На `lg` (1024–1279) — одна центрированная колода + легенда совпадения `MatchLegend`
    (`hidden lg:flex`).
  - На `xl`+ (≥1280, где основной области хватает на две колонки) — **two-pane**: слева
    колода (`xl:grid-cols-[minmax(0,440px)_minmax(0,1fr)]`), справа
    [`VacancyInsights`](src/components/swipe/vacancy-insights.tsx) — bento-панель «почему
    подходит» по верхней карточке (`sortedItems[0]`): работодатель, metric-карточка
    оценки, «сильные стороны» / «над чем подумать» из `match.strengths/gaps`, ключевые
    факты и «Подробнее». Панель использует только данные из стора (AI-match + вакансия),
    без доп. запросов; скрыта ниже `xl` (`hidden xl:flex`).
  - **Ширину самой карточки не меняли:** её считает формула в
    [`swipe-deck.tsx`](src/components/swipe/swipe-deck.tsx) под высоту вьюпорта.
  - Дизайн опирается на тренды-2026 (master–detail, bento-сетка, metric-карточка оценки
    с цветовым статусом). Для локального просмотра без входа — dev-параметр
    `/?preview=feed` (только в dev-сборке, подставляет демо-профиль; см. `home-client`).
- **Отклики** ([`liked-page-client.tsx`](src/components/liked/liked-page-client.tsx)) —
  `lg:max-w-5xl`, карточки писем сеткой `lg:grid-cols-2`.
- **Кабинет** ([`profile/page.tsx`](src/app/profile/page.tsx)) — `lg:max-w-4xl`, две
  колонки. Раскладка задана **явными** `lg:col-start/row-start` (а не перегруппировкой
  DOM), поэтому мобильный порядок карточек сохранён бит-в-бит: на десктопе слева лимиты
  + аккаунт + тема, справа резюме (`lg:row-span-3`); на мобиле — прежняя стопка. Пэйвол
  (`PaywallDialog`) — модалка, работает поверх кабинета без отдельной desktop-вёрстки.

Приём с явным grid-placement (вместо группировки в колоночные `<div>`) — ключевой:
он даёт двухколоночный десктоп, не меняя порядок и разметку мобильной версии.
...
Главная
  └─ useVacancies(filters) ─→ /api/vacancies ─(hh.ru, app-token)→ items[] (страницы по 30)
       ...
       ├─ useMatchScores(окно 18, батчи по 5) ─→ /api/match (дотягивает описание
       │    + key_skills по id) ─(ИИ)→ store.matches
...
### Фаза 3 — страницы «профессия + город» `/vakansii/{profession}/{city}`

- **`CITIES`** в [catalog.ts] — курируемый список из 16 крупных городов с реальными
  hh.ru `areaId` и **предложным падежом** (`prepositional`: «в Москве») для титулов
  «Вакансии {genitive} {prepositional}».
- **`COMBOS`** — явный allowlist комбинаций. Сейчас это `COMBO_PROFESSION_SLUGS`
  (топ-11 профессий) × `COMBO_CITY_SLUGS` (Москва + СПб + Екатеринбург) = **33
  страницы**. Маршрут `src/app/vakansii/[profession]/[city]/page.tsx` использует
  **`dynamicParams=false`** + `generateStaticParams = COMBOS`, поэтому генерится
  ровно этот набор, а всё остальное (`comboExists` → `notFound()`) отдаёт 404.
  **Так число страниц жёстко ограничено** (всего сайт: 26 профессий + хаб + 33
  комбо ≈ 60 — осознанный, индексируемый объём для молодого домена).
- Данные те же: `getProfessionLanding(prof, city.areaId)` (fetch уже принимал `area`).
  `buildProfessionFaq(prof, data, city)` делает FAQ/тексты город-зависимыми.
  JSON-LD: Breadcrumb на 4 уровня + ItemList + FAQPage.
- Перелинковка: на странице профессии — блок «{genitive} по городам» (только для
  combo-профессий, через `comboCitiesForProfession`); на combo-странице — ссылки на
  другие города и обратно «по всей России». В [sitemap.ts] добавлены все `COMBOS`.

> **Расширение наборов.** Чтобы добавить города — дополнить `COMBO_CITY_SLUGS`
> (города уже есть в `CITIES`); профессии — `COMBO_PROFESSION_SLUGS`. Каждый новый
> slug = +N страниц на билде, поэтому растить набор осознанно, по данным GSC/Метрики.
> Остальные города из `CITIES` доступны data-слою для будущих батчей.
