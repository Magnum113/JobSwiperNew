...
- **`kind=match`** — строит `messages` через тот же `buildMatchMessages()` и
  ...
  `info`. Если вакансия уже была оценена/лайкнута, данные берутся из сохранённого
  снапшота таблицы `vacancies`, то есть из того же search-level объекта hh.ru,
  который ушёл в `/api/match`. Если снапшота нет, страница fallback-ом тянет
  детальную вакансию через `GET /vacancies/{id}` и явно предупреждает, что такой
  prompt может отличаться от реального match-запроса, потому что production
  скоринг использует короткий `snippet`, а не полное `description`.
