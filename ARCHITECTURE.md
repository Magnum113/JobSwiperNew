...
- **`COMBOS`** — явный allowlist комбинаций. Сейчас это `COMBO_PROFESSION_SLUGS`
  (топ-11 профессий) × `COMBO_CITY_SLUGS` (Москва + СПб) = **22 страницы**. Маршрут
  `src/app/vakansii/[profession]/[city]/page.tsx` использует **`dynamicParams=false`**
  + `generateStaticParams = COMBOS`, поэтому генерится ровно этот набор, а всё
  остальное (`comboExists` → `notFound()`) отдаёт 404. **Так число страниц жёстко
  ограничено** (всего сайт: 26 профессий + хаб + 22 комбо = 49, кап ≤50).
