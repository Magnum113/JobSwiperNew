...
APP_SESSION_SECRET=...         # секрет HMAC для подписанной app-session cookie Яндекса
```
...
замену текущей авторизации: hh.ru OAuth хранится отдельной связкой токенов,
привязанной к текущему app user id (Supabase `auth.users.id` для Google или
stable app id для Яндекса).
...
9. **Auth:** Google — встроенный Supabase provider через `@supabase/ssr`,
   `proxy.ts`, `getAll/setAll` cookies и verified `auth.getUser()`. Яндекс —
   direct OAuth2 + PKCE через `/api/auth/yandex/callback` и signed app-session
   cookie; не возвращать его обратно на Supabase custom provider.
