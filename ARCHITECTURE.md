...
Авторизация (Google/Яндекс) — пока заглушки (кнопки без реального OAuth).
...
| База данных | **Supabase** (Postgres) + `@supabase/supabase-js` | хранение всех данных пользователя |
...
`jobswiper-user-id`). Все остальные данные — в Supabase (раздел 8).
...
│       └── db/                       # CRUD в Supabase (state/profile/swipe/matches/cover-letter/reset)
├── components/
...
│   ├── profile/
│   │   ├── auth-buttons.tsx   # кнопки Google/Яндекс (заглушки)
...
└── lib/
    ...
    │   └── queries.ts         # слой запросов к БД (загрузка/сохранение)
    ├── db-sync.ts             # браузерные обёртки к /api/db/* + анонимный userId
    ├── hooks/
...
- **на диске** в `.hh-token.json` (переживает рестарт dev-сервера).

Логика `getAppToken()`:
1. Есть валидный токен в памяти → отдать.
2. Иначе прочитать `.hh-token.json`.
3. Иначе запросить новый, записать в память и на диск.
...
> При первом запуске у пользователя токен запросится автоматически. В репозитории
> уже лежит засеянный `.hh-token.json`, чтобы всё работало сразу.
...
`pullState` делает несколько ретраев на случай холодного старта.
...
### RLS и безопасность
RLS включён на всех таблицах, но политики **разрешающие** (`using (true)`) для роли
`anon` — потому что **авторизации пока нет**, а скоупинг по `user_id` делается на
уровне API. Это осознанный компромисс для MVP (advisors показывают ожидаемый WARN
`rls_policy_always_true`). При добавлении реальной авторизации нужно заменить
`using (true)` на `using ((select auth.uid()) = user_id)`. Смена резюме чистит
...
2. **hh.ru требует app-токен** — без него `403` с дата-центра. Токен кэшируется в
   `.hh-token.json`; не запрашивать часто («refresh too early»).
...
8. **Данные — в Supabase** (Postgres), не в localStorage. В браузере остаётся только
   анонимный `userId`. Доступ к БД — только через серверные `/api/db/*` (раздел 8.1).
9. **Авторизация — заглушка** (кнопки без реального OAuth) → RLS-политики
   разрешающие, скоупинг по `user_id` на уровне API.
...
- Реальный OAuth Google/Яндекс + RLS по `auth.uid()` вместо разрешающих политик
  (данные уже в Supabase, осталось привязать их к настоящему пользователю).
