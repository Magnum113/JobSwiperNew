...
### Соискательский OAuth hh.ru + импорт резюме (реализовано)
Помимо **application token** (серверный поиск вакансий) приложение умеет входить
**от имени соискателя** через hh.ru и импортировать его резюме в кабинет.

> ⛔ **ВАЖНО (статус на 2026-06):** hh.ru **закрыл API резюме соискателя
> 15.12.2025** — `GET /resumes/mine` и `GET /resumes/{id}` отдают `403 forbidden`
> обычным OAuth-приложениям (доступ остался только у B2B-партнёров с отдельным
> соглашением по базе резюме). Поэтому **авто-импорт резюме сейчас не работает**:
> вход через hh.ru проходит успешно (`/me` доступен, токены сохраняются), но шаг
> резюме падает в `outcome="error"` → пользователю показывается честное сообщение
> «загрузите резюме файлом/текстом». Сам код импорта корректен и заработает, если
> hh.ru вернёт доступ или приложение получит партнёрский доступ к резюме.

Реализован как **standalone app-session** (та же подписанная cookie-сессия, что у
Яндекса), а **не** как Supabase custom provider: hh.ru не OIDC, и нам нужны его
access/refresh-токены, которые Supabase-провайдер выбросил бы.
...
2. `GET /api/auth/hh/callback` ([route.ts](src/app/api/auth/hh/callback/route.ts)):
   - проверяет `state`, меняет `code`+`verifier` на токен через
     `POST https://api.hh.ru/token` (`grant_type=authorization_code`);
   - `GET https://api.hh.ru/me` → `id`, `email`, имя, `is_applicant`,
     **`resumes_url`** (не хардкодим — берём из ответа);
   - ставит подписанную cookie `jobswiper-app-session`
     (`id = stableProviderUserId("hh", me.id)`);
   - сохраняет токены в `hh_connections` (раздел 8.1);
   - тянет список резюме по `resumes_url`:
     **0** → `?hh=empty`; **1** → сразу маппит и сохраняет профиль
     (`?hh=imported`); **>1** → `?hh=choose` (выбор на клиенте).
3. На `/profile` компонент
   [hh-resume-import.tsx](src/components/profile/hh-resume-import.tsx) реагирует
   на `?hh=`: тост (imported/empty) или диалог выбора (choose), грузя
   `GET /api/hh/resumes` и импортируя `POST /api/hh/resumes/import {resumeId}`.

**Маппинг резюме → профиль** — [resume-map.ts](src/lib/hh/resume-map.ts), напрямую
из структурированных полей hh.ru (**без расхода AI-квоты**): `title` ← `title`,
`skills` ← `skill_set`, `experienceId` ← `total_experience.months`, `seniority` и
`summary` выводятся из опыта/профролей, `rawText` собирается из полей резюме (нужен
для матчинга и писем — `buildResumeContext` использует `rawText`).

**Токены пользователя** — `expires_in` ≈ 14 дней, refresh **одноразовый** (выдаёт
новую пару, старый refresh сразу инвалидируется).
[user-token.ts](src/lib/hh/user-token.ts) (`getValidHhUserToken`) рефрешит токен
только при истечении и **сразу перезаписывает пару** в `hh_connections`. Запросы к
hh.ru от имени пользователя идут через `hhUserGet` (`Authorization: Bearer` +
`HH-User-Agent`).

> ⚠️ Тот же `HH_CLIENT_ID/SECRET` обслуживает и `client_credentials` (app-token),
> и `authorization_code` (user-token). В кабинете dev.hh.ru у приложения должен
> быть прописан Redirect URI `https://jobswiper.ru/api/auth/hh/callback`
> (+ `http://localhost:3000/...` для dev); доступ на чтение резюме может требовать
> модерации приложения hh.ru. Нужен также `APP_SESSION_SECRET` (фолбэки — раздел 3).
> Применить миграцию `supabase/migrations/20260609_hh_connections.sql` в проекте
> Supabase JobSwiper.
...
| `/api/auth/hh/callback` | GET | `code`, `state`, OAuth errors | обмен code на токен hh.ru, `/me`, app-session, токены в `hh_connections`, импорт/выбор резюме |
| `/api/hh/resumes` | GET | hh app-session | `{ resumes }` — список резюме пользователя для выбора |
| `/api/hh/resumes/import` | POST | `{ resumeId }` | импорт выбранного резюме hh.ru в профиль |
...
| `hh_connections` | соискательские OAuth-токены hh.ru (`access_token`, `refresh_token`, `expires_at`, `hh_user_id`) для импорта/рефреша резюме | `user_id` |
...
> ⚠️ Потолок остаётся за квотой: на free `analyses = 30` ([plans.ts](src/lib/plans.ts)),
> поэтому максимум 30 вакансий на резюме получат %. Параллельность ускоряет выдачу
...
9. **Auth:** Google — встроенный Supabase provider через `@supabase/ssr`,
   ...
   **hh.ru** — direct OAuth2 + PKCE через свой `/callback` и signed app-session
   cookie; не возвращать их обратно на Supabase custom provider. hh.ru
   дополнительно тянет резюме: токены в `hh_connections`, refresh **одноразовый**
   (перезаписывать пару сразу), резюме маппится без AI-квоты (раздел 5).
...
- Ужесточить RLS-политики под финальную модель гостевого/авторизованного доступа;
  ...
  app-session-юзеров `/api/db/*` берут `userId` от клиента — импорт резюме hh.ru
  это обходит, делая сохранение профиля строго server-side по cookie).
- Реальный отклик на hh.ru через соискательский OAuth (`POST /negotiations`) —
  токены пользователя уже хранятся в `hh_connections`.
